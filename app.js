...
document.getElementById('astroForm').addEventListener('submit', function (event) {
    ...
    const index1 = Math.floor(Math.random() * victimCardCompliments.length);
    const fourth_message = victimCardCompliments[index1];

    const index2 = (name.length * surname.length * year) % recommendations.length;
    const fifth_message = recommendations[index2];

    const index3 = (day * month * year) % predictions.length;
    const sixth_message = predictions[index3];

    result.innerHTML = `${first_message} ${second_message} ${fourth_message} <br><br> Our Recommendation for you: ${fifth_message} <br><br> Your Future Prediction: ${sixth_message}`;
});
